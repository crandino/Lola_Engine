...
bool ModuleAudio::Init()
{
	...
	{
		DEBUG("SDL_INIT_AUDIO could not initialize! SDL_Error: %s\n", SDL_GetError());
		ret = false;
	}
	...
	if((init & flags) != flags)
	{
		DEBUG("Could not initialize Mixer lib. Mix_Init: %s", Mix_GetError());
		ret = false;
	}
...
bool ModuleAudio::PlayMusic(const char* path, float fade_time)
{
	...
	if(music == NULL)
	{
		DEBUG("Cannot load music %s. Mix_GetError(): %s\n", path, Mix_GetError());
		ret = false;
	}
	else
	{
		if(fade_time > 0.0f)
		{
			...
			{
				DEBUG("Cannot fade in music %s. Mix_GetError(): %s", path, Mix_GetError());
				ret = false;
			}
		}
		else
		{
			...
			{
				DEBUG("Cannot play in music %s. Mix_GetError(): %s", path, Mix_GetError());
				ret = false;
			}
		}
	}
...
unsigned int ModuleAudio::LoadFx(const char* path)
{
	...
	if(chunk == NULL)
	{
		DEBUG("Cannot load wav %s. Mix_GetError(): %s", path, Mix_GetError());
	}
	else
	{
		fx.push_back(chunk);
